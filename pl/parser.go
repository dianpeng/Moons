...
func (p *parser) parseRuleBody(prog *program) error {
	condJump := -1
	...
	if p.l.token == tkIf {
		...
		// Notes, the code generation here is kind of tricky, we essentially wrap
		// the conditional code into a try catch branch. This lets the runtime
		// handle condition evaluation exception as failure case which is useful
		// for us in case of rule filtering. You want the rule filter to continue
		// unless nothing can be matched instead of aborting when certain rule's
		// conditional check failed
		pushExcep := prog.patch(p.l)
		if err := p.parseExpr(prog); err != nil {
			return err
		}
		popExcep := prog.patch(p.l)

		{
			// else branch for the try exception handling, we just load a false on the
			// value stack to normalize the exceptional case as evaluation failure
		...
		// pop exception jumpping to here to avoid/skip the else handling branch
		prog.emit1At(
			p.l,
			popExcep,
			bcPopException,
		...
		// lastly, we emit a conditional jump via bcFilter bytecode
...
func (p *parser) parseRule() error {
	...
	if err := p.parseRuleBody(prog); err != nil {
		return err
	}
