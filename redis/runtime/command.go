...
import (
	...
	"github.com/dianpeng/moons/redis/util"
...
var (
	...
	methodProtoCommandLength   = pl.MustNewFuncProto("redis.command.length", "%0")
...
func (c *command) Method(name string, arg []pl.Val) (pl.Val, error) {
	...
	case "length":
		if _, err := methodProtoCommandLength.Check(arg); err != nil {
			return pl.NewValNull(), err
		}
